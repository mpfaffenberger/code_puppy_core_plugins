...
@pytest.fixture
def mod():
    m = importlib.import_module("code_puppy_core_plugins.notifier.register_callbacks")
    return importlib.reload(m)


@pytest.fixture
def macos_module():
    m = importlib.import_module("code_puppy_core_plugins.notifier._macos")
    return importlib.reload(m)


@pytest.fixture
def windows_module():
    m = importlib.import_module("code_puppy_core_plugins.notifier._windows")
    return importlib.reload(m)
...
def test_disabled_config_suppresses_notifications(monkeypatch, mod):
    ...
    asyncio.run(mod._on_post_tool_call("ask_user_question", {}, None, 0))
...
def test_pre_tool_call_ignores_non_interactive_tool(monkeypatch, mod):
    ...
    _enable(mod, monkeypatch)
    sent = _stub_send(mod, monkeypatch)
    asyncio.run(mod._on_pre_tool_call("bash", {"command": "ls"}))
    assert sent == []
...
def test_no_duplicate_pre_tool_and_run_end(monkeypatch, mod):
    ...
    sent = _stub_send(mod, monkeypatch)
    asyncio.run(mod._on_pre_tool_call("ask_user_question", {}))
    asyncio.run(mod._on_post_tool_call("ask_user_question", {}, "answer", 100))
    mod._on_agent_run_end()
...
def test_agent_run_end_suppressed_after_user_responded(monkeypatch, mod):
    ...
    _enable(mod, monkeypatch)
    sent = _stub_send(mod, monkeypatch)
    asyncio.run(mod._on_post_tool_call("ask_user_question", {}, "yes", 50))
    mod._on_agent_run_end()
    assert sent == []
...
def test_post_tool_call_non_interactive_leaves_flag_false(monkeypatch, mod):
    ...
    _enable(mod, monkeypatch)
    sent = _stub_send(mod, monkeypatch)
    asyncio.run(mod._on_post_tool_call("bash", {"command": "ls"}, "output", 10))
    mod._on_agent_run_end()
    assert len(sent) == 1
...
def test_public_api_delegates_to_dispatcher(monkeypatch):
    ...
    notifier_init = importlib.import_module("code_puppy_core_plugins.notifier")
    rc = importlib.import_module("code_puppy_core_plugins.notifier.register_callbacks")
    ...
    sys.modules["code_puppy_core_plugins.notifier.register_callbacks"] = rc
...
def test_notify_command_title_and_message(monkeypatch, mod):
    ...
    monkeypatch.setattr(mod, "emit_success", lambda msg: None)
