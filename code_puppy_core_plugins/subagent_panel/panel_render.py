...
def _row_lines(ordered, frame):
    ...
    Root rows carry the INVOKE AGENT badge (FORK for /fork-started runs,
    BACKGROUND for fire-and-forget background delegations); nested rows carry
    the tree elbow.
    ...
    for e, depth in ordered:
        ...
        if depth == 0:
            if e.get("background"):
                # Background rows are marked distinctly so a fire-and-forget
                # delegation is never mistaken for a blocking invocation.
                left.append(" \u26a1 BACKGROUND ", style=f"bold white on {color}")
            elif e.get("is_fork"):
