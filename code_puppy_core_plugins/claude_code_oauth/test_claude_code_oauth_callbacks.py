...
class TestReadPastedCallbackLine:
    """Regression coverage for the mid-run re-auth paste-back deadlock.

    When an expired token triggers re-auth *during* a live agent run, the
    persistent TUI key-listener owns stdin and a pasted callback URL is routed
    onto the PauseController steer queue — never sys.stdin. The reader must
    pull from the queue in that state, and fall back to stdin everywhere else.
    """

    def test_falls_back_to_stdin_when_not_persistent(self):
        from code_puppy.plugins.claude_code_oauth.register_callbacks import (
            _read_pasted_callback_line,
        )

        with (
            patch("code_puppy.messaging.run_ui.is_persistent", return_value=False),
            patch(
                f"{MOD}.read_available_stdin_line", return_value="from-stdin"
            ) as mock_stdin,
        ):
            assert _read_pasted_callback_line() == "from-stdin"
            mock_stdin.assert_called_once()

    def test_uses_stdin_at_idle_even_when_persistent(self):
        # Idle /claude-code-auth runs inside suspended_run_ui(): persistent is
        # True but no run is active, so stdin is free and must be used.
        from code_puppy.plugins.claude_code_oauth.register_callbacks import (
            _read_pasted_callback_line,
        )

        with (
            patch("code_puppy.messaging.run_ui.is_persistent", return_value=True),
            patch("code_puppy.messaging.run_ui.is_run_active", return_value=False),
            patch(
                f"{MOD}.read_available_stdin_line", return_value="idle-stdin"
            ) as mock_stdin,
        ):
            assert _read_pasted_callback_line() == "idle-stdin"
            mock_stdin.assert_called_once()

    def test_reads_queued_paste_during_active_run(self):
        from code_puppy.plugins.claude_code_oauth.register_callbacks import (
            _read_pasted_callback_line,
        )

        pc = MagicMock()
        pc.pop_next_steer_queued.return_value = (
            "http://localhost:8765/callback?code=abc&state=xyz"
        )
        with (
            patch("code_puppy.messaging.run_ui.is_persistent", return_value=True),
            ...
            patch(f"{MOD}.read_available_stdin_line") as mock_stdin,
        ):
            assert (
                _read_pasted_callback_line()
                == "http://localhost:8765/callback?code=abc&state=xyz"
            )
            # The whole point: stdin is NOT consulted during a live run.
            mock_stdin.assert_not_called()

    def test_reads_now_queue_and_requeues_leftovers(self):
        from code_puppy.plugins.claude_code_oauth.register_callbacks import (
            _read_pasted_callback_line,
        )

        pc = MagicMock()
        pc.pop_next_steer_queued.return_value = None
        pc.drain_pending_steer_now.return_value = ["callback-url", "real steer"]
        with (
            patch("code_puppy.messaging.run_ui.is_persistent", return_value=True),
            patch("code_puppy.messaging.run_ui.is_run_active", return_value=True),
            patch(
            ...
            assert _read_pasted_callback_line() == "callback-url"
            # Unrelated steers the user typed must be handed back, not eaten.
            pc.request_steer.assert_called_once_with("real steer", mode="now")

    def test_empty_queues_return_none_without_touching_stdin(self):
        from code_puppy.plugins.claude_code_oauth.register_callbacks import (
            _read_pasted_callback_line,
        )
        ...
        pc = MagicMock()
        pc.pop_next_steer_queued.return_value = None
        pc.drain_pending_steer_now.return_value = []
        with (
            patch("code_puppy.messaging.run_ui.is_persistent", return_value=True),
            patch("code_puppy.messaging.run_ui.is_run_active", return_value=True),
            patch(
            ...
            assert _read_pasted_callback_line() is None
            mock_stdin.assert_not_called()
